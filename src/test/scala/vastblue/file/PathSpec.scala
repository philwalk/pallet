...
//    }
    if( isWindows ){
      ...
      val gdriveTests = if (gdrive.exists) { // should NOT really be a function of whether driver exists!
        List(
          ("/g","g:\\"),
          ("/g/","g:\\"),
        )
      ...
      describe ("# Path dospath test") {
        it("should correctly handle cygwin dospath drive designations, when present") {
         ...
         // val c = file.localpath.toLowerCase
            ...
            val df = normPath(d)
            val af = normPath(a)
            val sameFile = Files.isSameFile(af,df)
            if( !sameFile ){
              System.err.printf("expected[%s]\n",expected.toLowerCase)
              System.err.printf("file.localpath[%s]\n",file.localpath.toLowerCase)
              System.err.printf("error: expected[%s] not equal to dospath [%s]\n",expected.toLowerCase,file.localpath.toLowerCase)
        ...
        it("should correctly handle path toString") {
          ...
          for( (fname,expected) <- upairs ){
            ...
            if (nonCanonicalDefaultDrive){
              ...
              assert(std.endsWith(expected)) // in this case, there should also be a cygdrive prefix (e.g., "/c")
            } else {
              if(exp != std){
      ...
      describe ("# Path consistency") {
        it("round trip conversions should be consistent") {
          for( fname <- (pathToStringPairs.toMap.keySet ++ pathDospathPairs.toMap.keySet).toList.distinct.sorted ){
            if( fname == "/q/file" ){
              hook += 1
            }
            ...
            //val (k1,k2) = (f1.key,v.key)
              val sameFile = Files.isSameFile(f1,v)
              if (!sameFile){
