...
class FileSpec extends AnyFunSpec with Matchers with BeforeAndAfter {
  ...
  */
  ...
  lazy val gdriveTests = if (gdrive.exists) { // should NOT really be a function of whether driver exists!
    List(
      ("/g","g:\\"),
      ("/g/","g:\\"),
    )
  ...
  describe("File") {
    ...
    describe ("#tilde-in-path-test") {
      ...
      it("should NOT see file in user home directory if NOT present") {
        testfileb.delete()
        val ok = !testfileb.exists
        if (ok) println(s"delete() successfull, and correctly detected by 'exists' method on path '$testfileb'")
    ...
    if( isWindows ){
      ...
      describe ("# File name consistency") {
        it("round trip conversions should be consistent") {
          for( fname <- (toStringPairs.toMap.keySet ++ fileDospathPairs.toMap.keySet).toList.distinct.sorted ){
