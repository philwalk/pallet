...
trait TimeExtensions {
  ...
  */
  ...
  extension(ldt:java.time.LocalDateTime){
    def atStartOfDay():LocalDateTime = ldt.withHour(0).withMinute(0).withSecond(0).withNano(0) // atStartOfDay(zoneid)
...
//    }
    ...
    def withDayOfWeek(dow:java.time.DayOfWeek):LocalDateTime = d.`with`(TemporalAdjusters.next(dow))
