...
object pathextend {
  ...
  extension(p: Path) {
    ...
    def noDrive: String = p.norm.replaceAll("^/?[A-Za-z]:?/","/") // toss Windows drive letter, if present
    ...
    def age: String = { // readable description of lastModified
      ...
      if( lastModHours <= 24.0 ){
        "%1.2f hours".format(lastModHours)
      } else
      if( lastModDays <= 365.25 ){
    ...
    def trimmedSql: Seq[String] = lines.map { _.replaceAll("\\s*--.*","") }.filter{ _.trim.length > 0 }
    ...
    def withWriter(charsetName: String=DefaultEncoding, append: Boolean = false)(func: PrintWriter => Any): Unit = {
  ...
  extension(f: JFile) {
    ...
    def byteArray: Array[Byte] = bytes // alias
    def getBytes(encoding: String="utf-8"): Array[Byte] = contentAsString.getBytes(Charset.forName(encoding))
    ...
    def contentWithEncoding(encoding: String): String = f.path.linesWithEncoding(encoding).mkString("\n")
...
//          new PrintWriter(new FileWriter(f, charset, append))
  ...
  def _chmod(p: Path, permissions: String, allusers:Boolean): Boolean = Paths._chmod(p, permissions, allusers)
  ...
  * Filter for directories to be descended and/or
  ...
  */
  ...
  def autoDetectDelimiter(sampleText:String,fname:String,ignoreErrors:Boolean=true):String = {
    ...
    case _ if ignoreErrors =>
      ""
    case _ =>
      sys.error(s"unable to choose delimiter: tabs[$tabs], commas[$commas], semis[$semis], pipes[$pipes] for file:\n[${fname}]")
    }
  }
...
//  }
  ...
  def dropDotSuffix(s: String): String = if (!s.contains(".")) s else s.reverse.dropWhile(_ != '.').drop(1).reverse
