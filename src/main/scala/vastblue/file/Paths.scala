...
//import vastblue.pathextend.PosixDriveLetterPrefix
//import vastblue.file.Files.*
...
object Paths {
  ...
  def get(dirpathstr: String, subpath: String): Path = {
    ...
    val subtest = JPaths.get(subpath)
    if (subtest.isAbsolute){
  ...
  lazy val CygdrivePattern = "/([a-zA-Z])(/.*)?".r
  // there are three supported filename patterns:
  //    non-windows (posix by default; the easy case)
  //    windows semi-absolute path, with default drive, e.g., /Windows/system32
  ...
  def get(fnamestr: String): Path = {
    ...
    if (notWindows){
      ...
      val normpath = applyMountMap(_normpath) // if mounted, not pathRelative any longer
      val dd = defaultDrive.toUpperCase
      ...
      case pstr if pstr.startsWith("/") => // semi-absolute path, with no drive letter
        // semi-absolute paths are interpreted as being on the current-working-drive,
        ("", dd, pstr)
      ...
      if (literalDrive.nonEmpty) {
        // no need for cygpathM, if drive is unambiguous(?? is that correct? ; yes, if symlinks (and mounts?) are set correctly
        val fpath = if (fpstr.endsWith(":") && fpstr.take(3).length == 2) {
          cwd
        } else {
          jget(fpstr)
        }
        ...
        // TODO: fast enhanced isAbsolute that recognizes a bare drive letter as ambiguous (isAbsolute can be very slow)
        val cstr = fpstr // cygpathM(fpstr)
        jget(cstr)
      }
    }
  }

  // if mountMap has entries, apply it to `path`.
  def applyMountMap(path: String): String = {
    if (path.startsWith("q:")){
      vastblue.pathextend.hook += 1
    }
    var nup = path
    val (dl, segments) = pathSegments(path)
    if (segments.nonEmpty) {
      var firstSeg = s"/${segments.head}"

      val mounts = mountMap.keySet.toArray
      val mounted = mounts.find { (s: String) =>
        sameFile(s, firstSeg)
      }
      if (mounted.nonEmpty) {
        firstSeg = mountMap(firstSeg)
        nup = (firstSeg :: segments.tail.toList).mkString("/")
      }
    }
    nup
  }
  def getDriveLetter(str: String): String = {
    str.take(2).toList match {
      case letter :: ':' :: Nil =>
        s"$letter" // letter is a char, convert to String
      case _ =>
        "" // false
    }
  }
  def dropTrailingSlash(s: String): String = {
    s.reverse.dropWhile( (c:Char) => c =='/' || c == '\\').reverse
  }
  ...
  def expandTilde(s: String): String = {
    ...
    case false => s
    case true =>
      s.replaceFirst("~", userHome).replace('\\', '/')
    }
  }
  def userHome = sys.props("user.home").replace('\\', '/')

  def isSameFile(p1: Path, p2: Path): Boolean ={
    JFiles.isSameFile(p1, p2)
  }

  // get path to binaryName via 'which' or 'where'
  def where(binaryName: String): String = {
    if (isWindows) {
      // some winshell paths are invisible to java
      // where.exe provides paths that java can see
      // prefer binary with .exe extension, ceteris paribus
      val head = execBinary("where", binaryName).partition( _.endsWith(".exe")) match {
      case (exe,nonexe) if exe.nonEmpty =>
        exe.headOption.getOrElse("")
      case (_, nonexe) =>
        nonexe.headOption.getOrElse("")
      }
      if (head.isEmpty){
        ""
      } else {
  ...
  def notWindows = java.io.File.separator == "/"
  def isWindows = !notWindows
  def exeSuffix = if (isWindows) ".exe" else ""
  def osName = sys.props("os.name")
  def systemDrive = if (notWindows) "" else envOrElse("SYSTEMDRIVE", javaHome.take(2))

  lazy val winshellBashExes = Seq(
    "c:/msys64/usr/bin/bash.exe",
    "c:/cygwin64/bin/bash.exe",
  )
  def bashPath: String = {
    val inPath = where(s"bash${exeSuffix}")
    if (!isWindows) { inPath } else {
      inPath match {
        case "" | "C:/Windows/System32/bash.exe" =>
          winshellBashExes.find { (s: String) =>
  ...
  lazy val shellRoot: String = {
    if (notWindows) "/" else {
      val guess = bashPath.replaceFirst("(/usr)?/bin/bash.*","")
  ...
  lazy val shellRootPath = Paths.get(shellRoot)
  ...
  def defaultDrive: String = {
    if (notWindows) "" else {
      val h2 = here.take(2)
      val dl = h2.replaceAll("/", "").take(1)
      dl.toLowerCase
    }
  }
  ...
  def findAllInPath(prog: String, findAll: Boolean = true): Seq[Path] = {
    val progname = prog.replace('\\','/') match {
    case "/" => "/"
    case str => str.split("/").last // remove path from program, if present
    }
  ...
  def osType: String = osName.takeWhile(_!=' ').toLowerCase

  lazy val winshellFlag = {
     isDirectory(shellBaseDir) && (isCygwin || isMsys64 || isGitSdk64)
  }
  def isDirectory(path: String): Boolean = {
    Paths.get(path).toFile.isDirectory
  }

  lazy val programFilesX86: String = Option(System.getenv("ProgramFiles(x86)")) match {
    case Some(valu) => valu
    case None => "c:/Program Files (x86)"
  }
  ...
  def isCygwin = unameshort.toLowerCase.startsWith("cygwin")
  def isMsys64 = unameshort.toLowerCase.startsWith("msys")
  def isMingw64 = unameshort.toLowerCase.startsWith("mingw")
  def isGitSdk64 = unameshort.toLowerCase.startsWith("git-sdk")
  def isGitbash = unameshort.toLowerCase.startsWith("gitbash")
  ...
  def unameshort = unamefull.toLowerCase.replaceAll("[^a-z0-9].*","")
 ...
 // val letters = driveLettersLc.toArray
  ...
  def asLocalPath(str:String) = if (notWindows) str else str match {
  ...
  def norm(str:String) = str.replace('\\', '/') // Paths.get(str).normalize.toString.replace('\\', '/')

  lazy val mountMap: Map[String, String] = {
//    printf("shellRoot[%s]\n", shellRoot)
    if (notWindows || shellRoot.isEmpty ) {
      Map.empty[String, String]
    } else {
      vastblue.Platform.mountMap
      // readWinshellMounts
    }
  }
  ...
  def cygpathM(pathstr: String): String = {
    ...
    val tupes: Option[(String, String)] = mountMap.find { case (k,v) =>
  ...
  lazy val fsep = java.io.File.separator
  lazy val psep = sys.props("path.separator")
  ...
  */
  ...
  def sameFile(s1: String, s2: String): Boolean = {
    s1 == s2 || {
      // this branch addresses filesystem case-sensitivity
  ...
  def pathSegments(path: String): (String, Seq[String]) = {
    ...
    case s if s.endsWith(":") => (path.take(2), path.drop(2))
    case s => ("", path)
    }
