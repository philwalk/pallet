...
import vastblue.file.Util.readContentAsString
import java.io.{PrintWriter, OutputStreamWriter, FileWriter}
import scala.sys.process.*

// code common to scala3 and scala2.13 versions of pallet
object Util {
  ...
  def norm(p: Path): String    = nativePathString(p)
  def lcnorm(p: Path): String  = norm(p).toLowerCase
...
//  }
  ...
  */
  def renameFileUnique(srcfile: Path, newfile: Path, backupDir: Path): (Boolean, Option[Path]) = {
    if (isWindows) {
      ...
      assert(norm(srcfile) != norm(newfile))
    }
  ...
  */
  def withBasenameSuffix(p: Path, str: String, _newBasename: String = ""): Path = {
    ...
    if (newBasename.endsWith(str)) sys.error(s"[${norm(p)}] already has pre-suffix [$str]")
  ...
  */
  ...
  def suffix(p: Path): String = {
    ...
    name.reverse.takeWhile(_ != '.') match {
    case ""  => ""
    case str => str.reverse
    }
  }
...
//  }
  ...
  def readLinesAnyEncoding(p: Path, encoding: String = "utf-8"): Seq[String] = {
    readLinesIgnoreEncodingErrors(p, encoding).toSeq
  }
  ...
  private[vastblue] def _quikDate(s: String): DateTime = {
    _quikDateTime(s.take(10))
  }

  private[vastblue] def _quikDateTime(s: String): DateTime = {
