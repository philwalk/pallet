...
//    if( timeDebug ) System.err.print("parse(str=[%s], format=[%s]\n".format(str,format))
      ...
      _datestr match {
        ...
        case _ =>
          ...
          if (_datestr.matches("""2\d{7}""")) {
            ...
            val fixed =
              _datestr.replaceAll("""(....)\D(..)\D(..)\.(\d\d)(\d\d)""", "$1-$2-$3 $4:$5:00")
            LocalDateTime.parse(fixed, datetimeFormatter6)
          } else {
  ...
  def numerifyNames(datestr: String) = {
    val noweekdayName = datestr.replaceAll(
  ...
  lazy val mmddyyyyTimePattern3tz: Regex =
    """(\d{1,2})\D(\d{1,2})\D(\d{4})\D(\d\d):(\d\d):(\d\d)\D(-?[0-9]{4})""".r
  ...
  lazy val yyyymmddPatternWithTime3: Regex =
  ...
  class RichString(val s: String) extends AnyVal {
    ...
    def toDateTime: LocalDateTime = parseDateString(
      str
    ) // LocalDateTime.parse(str,selectZonedFormat(str))
...
//    def toLocalDateOption                 = toOption(toLocalDate)
    ...
    def dateTimeFormat(format: String): String =
      s.format(DateTimeFormatter.ofPattern(format)) // .parseDateTime(s)
    def localDateTimeFormat(format: String): String =
      s.format(DateTimeFormatter.ofPattern(format)) // .parseLocalDate(s)
  }
