...
//  lazy val nowdate = now.toString("yyyy-MM-dd")
  ...
  lazy val BadDate: LocalDateTime = yyyyMMddHHmmssToDate(List(1900,01,01))
    ...
    if (p.isFile) {
      ...
      for ((row, i) <- rows.zipWithIndex) {
        if (i > 0) {
          ...
          val format = ChronoParse(rawline)
          if (verbose) {
            printf("rawlin: [%s]\n", rawline)
            printf("   format: [%s]\n", format.toString)
          } else {
  ...
  def convertTimestamps(files: Seq[Path]): Seq[ChronoParse] = {
    for {
      p <- files
      if p.isFile
...
//        val midx = cleanFields.indexWhere { (s: String) => s.matches("(?i).*[JFMASOND][aerpuco][nbrylgptvc][a-z]*.*") }
  ...
  def cleanPrep(rawdatetime: String): (Seq[String], String, String, Int, Int) = {
